...
def perfect_square(score):
    if math.sqrt(score) == round(math.sqrt(score)):
        return True
    else:
        return False

def next_perfect_square(score):
    return pow(int(math.sqrt(score)) + 1, 2)
# END PROBLEM 4
...
def always_roll(n):
    ...
    assert n >= 0 and n <= 10
    # BEGIN PROBLEM 6
    def diceroll(player_score, opponent_score):
        return n
    return diceroll
    # END PROBLEM 6
